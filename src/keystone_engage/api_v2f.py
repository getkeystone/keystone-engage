...
load corpus, index embeddings, wire dispatch and coordination.

When KEYSTONE_MULTI_AGENT=true, /engage routes through the Coordinator
(5-phase specialist pipeline with event bus). When false (default),
/engage routes through the v1 EngageOrchestrator (single-agent path).

Both paths share the same audit chain, dispatcher, and task store.
"""
...
async def _create_event_bus():
    """Create and connect the EventBus if NATS is reachable."""
    try:
...
async def lifespan(app: FastAPI):
    ...
    if settings.multi_agent:
        # Multi-agent path: coordinator with event bus
        _event_bus = await _create_event_bus()
        from keystone_engage.coordinator import Coordinator
        _handler = Coordinator(
            audit=audit,
            dispatcher=dispatcher,
            task_store=task_store,
            event_bus=_event_bus,
        )
        logger.info("Pipeline: multi-agent coordinator (5-phase)")
    else:
        # v1 path: single-agent orchestrator
        _handler = EngageOrchestrator(
            audit=audit,
            dispatcher=dispatcher,
            task_store=task_store,
        )
        logger.info("Pipeline: v1 orchestrator (single-agent)")
...
async def health() -> HealthResponse:
    ...
    return HealthResponse(
        version=__version__,
        pipeline="multi-agent" if settings.multi_agent else "v1",
    )


@app.post("/engage", response_model=EngageResponse)
async def engage(request: EngageRequest) -> EngageResponse:
    assert _handler is not None, "Handler not initialized"
